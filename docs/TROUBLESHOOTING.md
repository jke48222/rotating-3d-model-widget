# rotating-3d-model — Troubleshooting

### The widget doesn't appear
- Make sure Übersicht is running, then use its menu-bar icon → **Refresh all**.
- Confirm `rotating-3d-model.widget` is in
  `~/Library/Application Support/Übersicht/widgets/`.
- Re-run `./install.sh`.

### Images or assets don't load
- Keep the `rotating-3d-model.widget` **folder intact**. Übersicht serves bundled assets
